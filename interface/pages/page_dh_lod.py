...
from interface.components import create_title_page, create_back_button, create_button, create_subtitle_page
from settings import LIST_WIDTH, LIST_HEIGHT, WINDOW_HEIGHT, PROGRESS_BAR_WIDTH, DEFAULT_PREFERENCES_KEY_PATH, DISTANT_HORIZONS_LOD_FOLDER_ID, MC_DH_SERVER_DATA_FOLDER
from interface.text import TEXT_LOD_TITLE, TEXT_LOD_SUBTITLE, TEXT_BTN_DOWNLOAD
from backend.local_handler import LocalHandler
from backend.api.google_handler import GoogleDriveHandler
from tkinter import messagebox
import threading
from utils.custom_outputs import print_done, print_info
from interface.components import CustomCheckBox
import os
from backend.verifications import Verificator


class PageDHLOD(ctk.CTkFrame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller
        self.configure(height=WINDOW_HEIGHT)

        self.title : ctk.CTkLabel = create_title_page(master=self, text=TEXT_LOD_TITLE, relx=0.5)
        self.subtitle : ctk.CTkLabel = create_subtitle_page(master=self, text=TEXT_LOD_SUBTITLE, relx=0.5, rely=0.12)
        ...
        self.frame_list_missing.place(relx=0.5, rely=0.45, anchor="center")
        ...
        self.label_total_files.place(relx=0.5, rely=0.16, anchor="center")

        # Ruta de instalación local para los archivos
        preferences = Verificator.preferences
        server_name : str = Verificator.server_data["name"]
        folder_name = server_name.replace(" ", "+")
        pref_mc_base_dir = preferences.get(DEFAULT_PREFERENCES_KEY_PATH)
        self.local_folder_path = os.path.join(pref_mc_base_dir, MC_DH_SERVER_DATA_FOLDER, folder_name)
        ...
        self.label_file_downloading.place(relx=0.02, rely=0.755, anchor="w")
        ...
        self.progress_bar.place(relx=0.02, rely=0.785, anchor="w")
    ...
    def reload_data_missing(self):
        ...
        self.missing_lods = LocalHandler.get_missing_files(cloud_folder_id=DISTANT_HORIZONS_LOD_FOLDER_ID, local_folder_path=self.local_folder_path)

        for file in self.missing_lods:
            ...
            self.checkboxes.append({'checkbox': checkbox, 'name': file['name'], 'var': var, 'mimeType':file['mimeType']}) # Guardar la referencia al checkbox y al archivo
    ...
    def execute_download(self):
        if messagebox.askokcancel("Confirmar descarga", f"Se han seleccionado {self.quantity_checkboxes_checked} fichero/s ¿Desea continuar?"):
            ...
            for item in self.checkboxes:
                ...
                if checkbox._variable.get() != "0":
                    self.download_file_list.append({"id":checkbox._variable.get(), "name":item["name"], "mimeType":item["mimeType"]})
                    print_info(f"Archivo {item["name"]} preparado para descargar. Tipo: {item["mimeType"]}")
