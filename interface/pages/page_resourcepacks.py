...
from settings import LIST_WIDTH, LIST_HEIGHT, WINDOW_HEIGHT, PROGRESS_BAR_WIDTH, DEFAULT_PREFERENCES_KEY_PATH, RESOURCEPACKS_FOLDER_ID
from interface.text import TEXT_RESOURCEPACKS_TITLE, TEXT_BTN_DOWNLOAD
...
class PageResourcepacks(ctk.CTkFrame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller
        self.configure(height=WINDOW_HEIGHT)

        self.title : ctk.CTkLabel = create_title_page(master=self, text=TEXT_RESOURCEPACKS_TITLE, relx=0.5)
        ...
        self.local_folder_path = os.path.join(pref_mc_base_dir, "resourcepacks")
    ...
    def reload_data_missing(self):
        ...
        self.missing_resourcepacks = LocalHandler.get_missing_files(cloud_folder_id=RESOURCEPACKS_FOLDER_ID, local_folder_path=self.local_folder_path)

        for file in self.missing_resourcepacks:
